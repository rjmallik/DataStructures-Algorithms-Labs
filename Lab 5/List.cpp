...
void List::cleanup() {
    ...
    while (current != backDummy && current->next != backDummy) {
        Node* runner = current->next;
        while (runner != backDummy) {
            if (current->data == runner->data) {
                Node* duplicate = runner;
                runner = runner->next; // Move runner ahead before deleting the duplicate node
                duplicate->prev->next = duplicate->next;
                duplicate->next->prev = duplicate->prev;
                delete duplicate; // Properly delete the node to prevent memory leaks
                num_elements--; // Update the count of elements in the list
            } else {
                runner = runner->next;
            }
        }
    ...
    // Recalculate the cursor positions to ensure they are accurate after cleanup
    beforeCursor = frontDummy;
    afterCursor = frontDummy->next;
    int currentPosition = 0;
    while (currentPosition < pos_cursor && afterCursor != backDummy) {
        beforeCursor = afterCursor;
        afterCursor = afterCursor->next;
        currentPosition++;
    }
